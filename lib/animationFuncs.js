import { stagger } from "motion/react";

/**
 * @typedef SectionAnimFuncPropType
 * @property {import("motion/react").AnimationScope<HTMLDivElement>} scope
 * @property {ReturnType<import("motion/react").useAnimate>[1]} animate
 */

const anim = {
  blurOut: { opacity: 0, filter: "blur(8px)", y: 10 },
  dur0: { duration: 0 },
  blurIn: { opacity: 1, filter: "blur(0px)", y: 0 },
};

const animFns = {
  /**
   *
   * @param {SectionAnimFuncPropType} param0
   */
  async homeServicesSection({ animate }) {
    // Instant reset
    await Promise.all([
      animate("#service-section-heading", { y: 30, opacity: 0 }, { duration: 0 }),
      animate("#service-section-description", { y: 20, opacity: 0, filter: "blur(4px)" }, { duration: 0 }),
      animate(".expandable-card", { y: 44, opacity: 0 }, { duration: 0 }),
    ]);

    animate("#service-section-heading", { y: 0, opacity: 1 }, {
      duration: 0.55,
      ease: [0.33, 1, 0.68, 1],
    });

    // Description fades up with slight blur
    animate("#service-section-description", { y: 0, opacity: 1, filter: "blur(0px)" }, {
      duration: 0.6,
      delay: 0.15,
      ease: "easeOut",
    });

    // Cards cascade up with stagger — start after heading is mostly in
    animate(".expandable-card", { y: 0, opacity: 1 }, {
      duration: 0.5,
      delay: stagger(0.08, { startDelay: 0.25 }),
      ease: [0.33, 1, 0.68, 1],
    });
  },

  /**
   *
   * @param {SectionAnimFuncPropType} param0
   */
  async homeProcessSection({ animate }) {
    await Promise.all([
      animate("#process-section-heading",     { x: 52, opacity: 0 },  { duration: 0 }),
      animate("#process-section-description", { x: 52, opacity: 0 },  { duration: 0 }),
      animate(".process-step",                { x: -44, opacity: 0 }, { duration: 0 }),
    ]);

    // Heading + subtitle slide in from right
    animate("#process-section-heading", { x: 0, opacity: 1 }, {
      duration: 0.65,
      ease: [0.33, 1, 0.68, 1],
    });
    animate("#process-section-description", { x: 0, opacity: 1 }, {
      duration: 0.55,
      delay: 0.12,
      ease: "easeOut",
    });

    // Steps cascade in from the left (ProcessTimeline owns circle/line visuals)
    animate(".process-step", { x: 0, opacity: 1 }, {
      duration: 0.5,
      delay: stagger(0.13, { startDelay: 0.18 }),
      ease: [0.33, 1, 0.68, 1],
    });
  },

  async homeConcernSection({ animate }) {
    await Promise.all([
      animate("h2", { y: 40, opacity: 0 }, { duration: 0 }),
      animate("#process-section-description", { y: 24, opacity: 0, filter: "blur(4px)" }, { duration: 0 }),
      animate("#concern-marquee", { y: 40, opacity: 0 }, { duration: 0 }),
    ]);

    await animate("h2", { y: 0, opacity: 1 }, {
      duration: 0.65,
      ease: [0.33, 1, 0.68, 1],
    });

    animate("#process-section-description", { y: 0, opacity: 1, filter: "blur(0px)" }, {
      duration: 0.55,
      ease: "easeOut",
    });

    animate("#concern-marquee", { y: 0, opacity: 1 }, {
      duration: 0.65,
      delay: 0.15,
      ease: [0.33, 1, 0.68, 1],
    });
  },

  async aboutMissionVisionSection({ animate }) {
    await Promise.all([
      animate("#mv-section-title", { y: 32, opacity: 0 }, { duration: 0 }),
      animate(".about-card",       { y: 52, opacity: 0 }, { duration: 0 }),
    ]);

    await animate("#mv-section-title", { y: 0, opacity: 1 }, {
      duration: 0.65,
      ease: [0.33, 1, 0.68, 1],
    });

    animate(".about-card", { y: 0, opacity: 1 }, {
      duration: 0.65,
      delay: stagger(0.18),
      ease: [0.33, 1, 0.68, 1],
    });
  },

  async aboutApproachSection({ animate }) {
    await Promise.all([
      animate("#approach-heading", { x: -44, opacity: 0 },              { duration: 0 }),
      animate("#approach-body",    { x: -32, opacity: 0, filter: "blur(4px)" }, { duration: 0 }),
      animate(".approach-card",    { x: 44,  opacity: 0 },              { duration: 0 }),
    ]);

    animate("#approach-heading", { x: 0, opacity: 1 }, {
      duration: 0.65,
      ease: [0.33, 1, 0.68, 1],
    });

    animate("#approach-body", { x: 0, opacity: 1, filter: "blur(0px)" }, {
      duration: 0.55,
      delay: 0.12,
      ease: "easeOut",
    });

    animate(".approach-card", { x: 0, opacity: 1 }, {
      duration: 0.52,
      delay: stagger(0.12, { startDelay: 0.18 }),
      ease: [0.33, 1, 0.68, 1],
    });
  },

  async homeFounderSection({ animate }) {
    await Promise.all([
      animate("#founder-photo",   { x: -52, opacity: 0 },               { duration: 0 }),
      animate("#founder-content", { x: 52,  opacity: 0 },               { duration: 0 }),
      animate(".founder-star",    { scale: 0, opacity: 0 },             { duration: 0 }),
      animate(".founder-para",    { y: 24, opacity: 0, filter: "blur(4px)" }, { duration: 0 }),
      animate("#founder-cta",     { y: 20, opacity: 0, scale: 0.94 },   { duration: 0 }),
    ]);

    // Phase 1 — photo and content block slide in simultaneously
    animate("#founder-photo", { x: 0, opacity: 1 }, {
      duration: 0.7,
      ease: [0.33, 1, 0.68, 1],
    });
    await animate("#founder-content", { x: 0, opacity: 1 }, {
      duration: 0.7,
      delay: 0.1,
      ease: [0.33, 1, 0.68, 1],
    });

    // Phase 2 — paragraphs stagger up with blur dissolve
    await animate(".founder-para", { y: 0, opacity: 1, filter: "blur(0px)" }, {
      duration: 0.5,
      delay: stagger(0.15),
      ease: [0.33, 1, 0.68, 1],
    });

    // Phase 3 — CTA bounces in
    animate("#founder-cta", { y: 0, opacity: 1, scale: 1 }, {
      duration: 0.55,
      ease: [0.34, 1.56, 0.64, 1],
    });

    // Phase 4 — stars sparkle in
    animate(".founder-star", { scale: 1, opacity: 1 }, {
      duration: 0.45,
      delay: stagger(0.18),
      ease: [0.34, 1.56, 0.64, 1],
    });
  },

  async homeFaqSection({ animate }) {
    await Promise.all([
      animate("#faq-left",    { x: -44, opacity: 0 }, { duration: 0 }),
      animate(".faq-content", { x: 44,  opacity: 0 }, { duration: 0 }),
      animate(".faq-item",    { y: 24,  opacity: 0 }, { duration: 0 }),
    ]);

    // Left panel slides in from left
    animate("#faq-left", { x: 0, opacity: 1 }, {
      duration: 0.65,
      ease: [0.33, 1, 0.68, 1],
    });

    // Accordion block slides in from right
    await animate(".faq-content", { x: 0, opacity: 1 }, {
      duration: 0.65,
      delay: 0.12,
      ease: [0.33, 1, 0.68, 1],
    });

    // Items stagger up
    animate(".faq-item", { y: 0, opacity: 1 }, {
      duration: 0.42,
      delay: stagger(0.1),
      ease: [0.33, 1, 0.68, 1],
    });
  },

  async homeCTASection({ animate }) {
    await Promise.all([
      animate("#cta-badge",   { y: 20, opacity: 0 }, { duration: 0 }),
      animate("#cta-heading", { y: 44, opacity: 0 }, { duration: 0 }),
      animate("#cta-sub",     { y: 24, opacity: 0, filter: "blur(6px)" }, { duration: 0 }),
      animate("#cta-buttons", { y: 20, opacity: 0, scale: 0.95 }, { duration: 0 }),
    ]);

    animate("#cta-badge", { y: 0, opacity: 1 }, {
      duration: 0.5,
      ease: [0.33, 1, 0.68, 1],
    });

    await animate("#cta-heading", { y: 0, opacity: 1 }, {
      duration: 0.75,
      delay: 0.12,
      ease: [0.33, 1, 0.68, 1],
    });

    animate("#cta-sub", { y: 0, opacity: 1, filter: "blur(0px)" }, {
      duration: 0.55,
      ease: "easeOut",
    });

    animate("#cta-buttons", { y: 0, opacity: 1, scale: 1 }, {
      duration: 0.55,
      delay: 0.15,
      ease: [0.34, 1.56, 0.64, 1],
    });
  },

  /**
   *
   * @param {SectionAnimFuncPropType} param0
   */
  async homeToAboutSection({ animate }) {
    await animate("#founder-message", { marginTop: 0 }, {duration: 0.6})
  }
};

export default animFns;
