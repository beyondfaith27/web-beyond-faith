...
const animFns = {
  ...
  async homeProcessSection({ animate }) {
    await Promise.all([
      ...
      animate(".process-line",                { scaleY: 0 },           { duration: 0 }),
    ]);
    ...
    // Steps cascade in from the left
    await animate(".process-step", { x: 0, opacity: 1 }, {
      duration: 0.5,
      delay: stagger(0.13, { startDelay: 0.18 }),
      ease: [0.33, 1, 0.68, 1],
    });

    // Connecting line draws itself downward
    animate(".process-line", { scaleY: 1 }, {
      duration: 1.1,
      ease: [0.33, 1, 0.68, 1],
    });

    // Circles pulse in sequence — bounce scale to highlight each step
    animate(".process-circle", { scale: [1, 1.22, 1] }, {
      duration: 0.38,
      delay: stagger(0.13),
